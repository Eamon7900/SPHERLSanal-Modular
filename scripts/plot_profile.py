...
# Usage: plot_profile.py [options] XMLFILE

# Reads in the XMLFILE which specifies how the plot is to be made and generates
# the plot of plots accordingly.
...
class DataSet:
  ...
  def load(self,options):
    ...
    #assume that all the combined binary files within range of dataset have profiles made
    ...
    if len(files)==0:
      raise Exception("no files found in range ["+str(self.start)+"-"
        +str(self.end)+"].  Did you run mkRadPro to generate profile text files?")
...
def main():
  ...
  if len(args)==0:
    ...
    #get root element
    if args[0][0:1] == "." or args[0][0:1] == "/" :  #If an absolute path is given, use it.
      tree=xml.parse(args[0]);
    else: #if an absolute path is not given, assume the XML file is in the config directory
      tree=xml.parse(paths.SPHERLSanalConfig + args[0])
