...
#Usage: dump.py [options] DUMPFILENAME

# Performs some basic tests on the Dump class. Provide a testdump file with
#DUMPFILENAME
...
#The below two lines should not needed when a proper install is done
...
class Dump:
  ...
  def _readHeaderBinary(self,eosFile=None):
    ...
    if self.eosStringLen>0:
      ...
      for i in range(self.eosStringLen):
        self.eosString+=str(struct.unpack('s',self.f.read(1))[0])
  ...
  def _adjustForPeriodBC(self):
    ...
    for n in range(len(self.varInfo)):
      if self.varInfo[n][1]==1:
        print("Current index " + str(n)) #DEBUG
  ...
  def getRectVarNames(self):
    ...
    varNames=[]
    for key in list(self._varIDs.keys()):
  ...
  def setRectVars(self):
    ...
    """
    print(range(len(self.vars)))
    for i in range(len(self.vars)):
      self.rectVars.append(self.getRectVar(i))
      print("Rectvars[" + str(i) + "] set to " + str(self.getRectVar(i))) #DEBUG
  ...
  def writeVTKFile(self,fileName,withGhost=True,eosFile=None,curvature=True
    ...
    pyevtk.hl.gridToVTK(fileName, x, y, z, cellData=cellData, pointData = {})
