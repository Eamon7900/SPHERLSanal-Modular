...
# usage: test_calculation.py [-h] [-f] [-m] [-r] [-k] [-p P] [-t T]
...
def main():
  ...
  if not options.m:
    for haveRef in haveRefCals:
      print("checking calculation against \""+paths.ref_calcsPath+"/"+haveRef+"\" calculation ...", end=' ')
...
def checkForRefCalcsAndRemake(options):
  ...
  haveRefCalcs={}
  for key in list(ref_calcs.refCalcs.keys()):
    
    #check that all model files are there
    haveRefCalc=True
    if not options.m:
      print("checking for reference calculations for \""+key+"\" calculation ... ", end=' ')
    ...
    if not haveRefCalc:
      if options.r or options.m:
        if not options.m :
          print("FAILED")
          print("  remaking \""+key+"\" reference calculations ... ", end=' ')
        else:
          print("remaking \""+key+"\" reference calculations ... ", end=' ')
        if key=='1DNA':
