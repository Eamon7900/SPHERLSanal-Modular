...
# Usage: make_2DSlices.py [options] BASEFILENAME[START-END]

# Slices the 3D dump files of "SPHERLS" into 2D slices. This is a front end for
# the "SPHERLSanal" program allowing multiple files to be sliced at once from
# START upto but not including END. Ex. there are a number of files in the
# current directory from run1_t00000000, to run1_t00012300, one could create 2D
# slice of all these files with the command make_2DSlices.py run1_t[0-12300] or
# make_2DSlices.py run1_t[0-*]. The "*" wild character is used to include all
# files upto the maximum integer starting from START.
