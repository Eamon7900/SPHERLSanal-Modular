...
# Usage: make_profiles.py [options] BASEFILENAME[START-END]

# Creates radial profiles from a range of combined binary files starting with
# BASEFILENAME with time step indices including START up to but not including
# END. Ex. there are a number of files in the current directory from
# run1_t00000000, to run1_t00012300, one could combine all these files with the
# command make_profiles.py run1_t[0-12300] or make_profiles.py run1_t[0-*]. The
# "*" wild character is used to include all files up to the maximum integer
# starting from START.

#Runs SPHERLSanal with -a cb flags on specified binary files.
