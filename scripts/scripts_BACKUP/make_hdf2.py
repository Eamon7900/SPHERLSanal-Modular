#!/usr/bin/env python
#TODO rename this... make_hdf2 is confusing and misleading when make_hdf is just a broken script
# Usage: make_hdf2.py [options] XMLFILE

# Creates one or more HDF files. XMLFILE gives all the settings needed. For an
# example of a configuration file see make_hdf_reference.xml under
# docs/templateXML.
...
class fileSet:
  ...
  def __checkSuppotedNodeAttributes(self,element):
    ...
    if element.tag not in list(self.supportedNodeAttributes.keys()):
  ...
  def makeHDFFiles(self,options):
    ...
    #write out time
    h5py.open(self.timeFile)
    h5py.openData("time",h5py.DFNT_FLOAT64,[len(times)])
    h5py.writeData(times)
    h5py.closeData()
    h5py.close()
  def convertDumpToHDF(self,dump):
    ...
    for data in self.data:
      ...
      h5py.openData(self.dataNames[n],h5py.DFNT_FLOAT64,self.dataShape[n])
      h5py.writeData(data)
      h5py.closeData()
      n+=1
    h5py.close()
