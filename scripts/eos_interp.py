...
# Usage: eos_interp.py [options] XMLFILE
#        eos_interp.py [options] EOSFILE1 EOSFILE2

# In the first form of invocation it interpolates in a set of equation of state
# files. XMLFILE gives all the settings needed to create multiple eos files used
# in SPHERLS. For an example of a configuration file see
# eos_interp_reference.xml under docs/templateXML. In the second usage case it
# can be used to compare two equation of state files.
...
class opacityTableManager:
  ...
  def __init__(self,opacityConfigFile=None):
    ...
    if opacityConfigFile!=None:
      ...
      fileCompositions={}
      for id in list(self.opacityFileNames.keys()):
