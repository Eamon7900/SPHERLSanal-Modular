#!/usr/bin/env python
#Broken, use make_hdf2.py instead
import sys
import paths
...
class hdfFile:
  ...
  def __makeInterpVars(self):
    ...
    for key in list(self.interpVars.keys()):
  ...
  def __makeVars(self):
    ...
    for n in range(len(self.variables)):
      ...
      for indepVar in self.variables[n].indep:
        if indepVar!=None:
          nNumDims+=1
          if indepVar in list(self.interpVars.keys()):
  ...
  def __interpolateToIndep(self,l,n,tmpIn,code):
    ...
    if self.variables[n].indep[l]==None:
      numPoints=1
    elif self.variables[n].indep[l] in list(self.interpVars.keys()):
