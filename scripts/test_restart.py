...
# usage: test_restart.py [-h] [-f] [-k] [-p P] [-t T]

# Tests that model restarts produce the same output as if the code had been run
# straight through.
...
def testRestarts(tmpDir,startModel,exePath,numProcs,options,numTimeSteps):
  ...
  #to perform test
  print("making a temporary directory, \""+tmpDir+"\" to store test data ...", end=' ')
  ...
  log=open("log.txt",'w')
  print("running \""+exePath+"\" for "+str(numTimeSteps+1)+" time steps ...", end=' ')
  ...
  print("combining binary dump \"./RestartTest2_t[0-*]\" for restart ...", end=' ')
  log=open("log.txt",'a')
  ...
  print("combining binary files for dump \"./RestartTest2_t[0-*]\" ...", end=' ')
