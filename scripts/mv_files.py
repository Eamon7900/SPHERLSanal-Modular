...
# Usage: mv_files.py [options] BASEFILENAME[START-END] NEWBASEFILENAME

# Moves a range of files starting with BASEFILENAME with time step indices
# startingat START upto and including END to NEWBASFILENAME+INDEX. Ex. there are
# a number of files in the current directory from run1_t00000000, to
# run1_t00012300, one could move all these files to the new locations
# run2_t00000000 through to run2_t00012300 with the command mv_files.py
# run1_t[0-12300] run2 or mv_files.py run1_t[0-*] run2. The "*" wild character
# is used to include all files upto the maximum integer starting from START.
# This command also moves files with additional suffixes, such as the
# distributed files, profiles, 2D slices etc., while maintianing the correct
...
def move_files(fileName,newBaseFileName,options):
  ...
  if(len(files)==0):
    #TODO this print statement contains an undefined variable
